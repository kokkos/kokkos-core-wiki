...
that makes the lambda work correctly with CUDA, similar to the `KOKKOS_INLINE_FUNCTION` macro. If you do not plan to build with CUDA, you may use `[=]` explicitly, but we find using the macro easier than remembering the capture clause syntax.
