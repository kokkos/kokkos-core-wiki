...
* If ``ExecPolicy`` is ``TeamVectorMDRange``, ``TeamThreadMDRange``, or ``ThreadVectorMDRange``, only a ``ReducerArgumentNonConst`` is allowed, and it must be a POD type with ``operator +=`` and ``operator =``.
